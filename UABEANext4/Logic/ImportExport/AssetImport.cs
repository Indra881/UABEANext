    ...
    private void ImportTextAssetLoop(AssetsFileWriter writer)
    {
        ...
        while (true)
        {
            ...
            if (eqSign != -1)
            {
                ...
                {
                    writer.Write(float.Parse(valueStr.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture));
                }
                ...
                {
                    writer.Write(double.Parse(valueStr.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture));
                }
